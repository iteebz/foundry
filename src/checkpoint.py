...
def load_hf_checkpoint(model, hf_model_name: str, cache_dir: str = None) -> None:
    ...
    try:
        ...
        raise ImportError("transformers required for checkpoint loading: pip install transformers")
    
    hf_model = GPT2LMHeadModel.from_pretrained(hf_model_name, cache_dir=cache_dir)
    hf_sd = hf_model.state_dict()
    
    foundry_sd = model.state_dict()
    foundry_keys = [k for k in foundry_sd.keys() if not k.endswith('.attn.bias')]
    ...
    for hf_key in hf_keys:
        foundry_key = hf_key.replace('transformer.h.', 'h.').replace('transformer.wte.', 'wte.').replace('transformer.wpe.', 'wpe.').replace('transformer.ln_f.', 'ln_f.')
