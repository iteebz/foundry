...
def get_alibi_slopes(n_heads):
    ...
    def get_slopes_power_of_2(n):
        start = 2 ** (-(2 ** -(torch.arange(n).float() / n)))
        return start
    ...
    if (n_heads & (n_heads - 1)) == 0:
        ...
        return torch.cat([
            ...
            get_alibi_slopes(int(2 * closest_power_of_2))[0::2][:n_heads - int(closest_power_of_2)]
        ])
...
class ALiBi(nn.Module):
    ...
    def forward(self, seq_len):
        ...
        bias = self.alibi_bias[:, :, :seq_len, :seq_len] * self.slopes
        return bias
