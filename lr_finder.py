...
def lr_find(model, get_batch, device, ctx, min_lr=1e-7, max_lr=10, num_steps=100, beta=0.98):
    ...
    model.train()
    optimizer = model.configure_optimizers(weight_decay=1e-1, learning_rate=min_lr, betas=(0.9, 0.95), device_type='cuda' if 'cuda' in str(device) else 'cpu')
...
def plot_lr_find(lrs, losses, out_path='out/lr_find.png'):
    ...
    plt.axvline(suggested_lr, color='r', linestyle='--', label=f'Suggested LR: {suggested_lr:.2e}')
    plt.legend()
...
def main():
    ...
    if model_type == 'v1':
        ...
        config = GPTConfig(n_layer=6, n_head=6, n_embd=384, block_size=block_size, vocab_size=vocab_size, bias=False, dropout=0.0)
    elif model_type == 'v2':
        from model_v2 import GPTConfig, GPT
        config = GPTConfig(n_layer=6, n_head=6, n_kv_head=2, n_embd=384, block_size=block_size, vocab_size=vocab_size, bias=False, dropout=0.0)
    else:
